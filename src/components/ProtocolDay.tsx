...
const HUNGER_RESPONSES: Record<string, string> = {
  ...
  "Alta": "Fome alta é normal no Dia 4. Seu corpo ainda está se adaptando. Faça um lanche proteico agora: iogurte grego + 1 fruta. Vai segurar até a próxima refeição. 🥣",
...
export default function ProtocolDay({ onNavigate }: { onNavigate: (screen: string) => void }) {
  ...
  const toggleMission = (id: number) => {
    ...
    setCheckedMissions(prev =>
      isChecking ? [...prev, id] : prev.filter(m => m !== id)
    );
    if (isChecking) {
  ...
  const handleComplete = () => {
    ...
    confetti({
      ...
      spread: 80,
      origin: { y: 0.6 },
      colors: ['#00C27A', '#00E5A0', '#ffffff'],
    });
  };

  const progress = checkedMissions.length / MISSIONS.length;
      ...
      <div className="p-5 pb-6">
        ...
        <header className="flex items-center gap-3 mb-5">
          ...
          <div className="flex-grow">
            <h1 className="font-bold text-base">Protocolo 1 · Dia 4 de 7</h1>
            <div className="flex gap-1.5 mt-1.5">
              {[1,2,3,4,5,6,7].map(i => (
                <div key={i} className={`h-2 flex-1 rounded-full transition-all ${i <= 4 ? 'bg-primary' : 'bg-border'}`} />
              ))}
            </div>
          </div>
          {xpTotal > 0 && (
        ...
        <div className="relative aspect-video bg-text-main rounded-2xl overflow-hidden mb-5 flex items-center justify-center">
          ...
          <motion.button
            whileTap={{ scale: 0.95 }}
            className="bg-primary/90 text-white p-4 rounded-full shadow-lg"
          >
            ...
            {MISSIONS.map(m => {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-grow">
                    <p className={`font-bold text-sm ${checked ? 'line-through text-text-muted' : ''}`}>
                      {m.title}
                    </p>
        ...
        {completed && (
          ...
          >
            ...
            <p className="text-xs text-text-muted mt-1">Dia 5 desbloqueado. Até amanhã!</p>
            <button
